...
export function getRandomWords(
  ...
  let wordsArr: string[] = generate({
    exactly: quantity,
    maxLength: 8,
  }) as string[];
  if (punctuation) {
    wordsArr[0] = firstLetterUpperCase(wordsArr[0]);
    let i = getRandomNumberBetween(2, 9);
    ...
    wordsArr[wordsArr.length - 1] = wordsArr[wordsArr.length - 1] =
      lastMarkArr[getRandomNumberBetween(0, lastMarkArr.length - 1)];
  }
  if (numbers) {
    let numberQuantityLeft = getRandomNumberBetween(
      quantity * 0.05,
      quantity * 0.3
    );
    let insertAt = -1;
    while (numberQuantityLeft > 0) {
      insertAt++;
      const numberLength = getRandomNumberBetween(1, 4);
      const generateNumber = getRandomNumberBetween(
        Number('1' + '0'.repeat(numberLength - 1)),
        Number('9'.repeat(numberLength))
      );
      insertAt += getRandomNumberBetween(
        1,
        Math.floor((quantity - insertAt) / numberQuantityLeft)
        );
        if (insertAt < wordsArr.length - 1) {
            wordsArr[insertAt] = String(generateNumber);
        } else {
            break;
        }
        numberQuantityLeft--;
...
export function getTypingResults(
  ...
  return {
    ...
    accuracy: Math.max(
      Number((((typed - mistype) / typed) * 100).toFixed(2)),
      0
    ),
    raw: Math.round(typed / 5 / timeTookInMin),
  };
}
// The purpose of this function is to round a number to a certain number of decimal places,
// with the number of decimal places varying based on the magnitude of the input:
...
export function getTimeSince(data: Date, strShort?: boolean) {
  var msPerMinute = 60 * 1000;
  var msPerHour = msPerMinute * 60;
  var msPerDay = msPerHour * 24;
  var msPerMonth = msPerDay * 30;
  var msPerYear = msPerDay * 365;
  var elapsed = Date.now() - data.getTime();
